...
### Loop可视化（Loop Visualization）
- 前端右侧栏的Loop状态展示模块
- **侧边栏缩略图**：以垂直节点列表的形式展示Loop的节点结构和当前执行状态
- **扩展模态框**：点击缩略图后弹出，以从上到下的节点图形式展示详细的Loop状态
- **下拉菜单**：用于切换显示不同的Loop定义
- **状态显示**：未激活（灰色）、CREATE,RUNNING（蓝色）、PAUSED（黄色）、COMPLETED（绿色）、FAILED（红色）
