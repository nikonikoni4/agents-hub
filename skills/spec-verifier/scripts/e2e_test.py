...
def test_complex_scenarios():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        session_file = create_test_session(session_dir, "test456", user_messages, ai_documents)
        ...
        # 读取会话数据
        session_data = read_session_data("test456", Path(temp_dir))
        ...
        # 验证结果
        # 用户需求包括：1. "我需要一个用户登录功能" 2. "还需要支持第三方登录" 3. "密码必须加密存储"
        assert len(user_requirements) == 3, f"用户需求数量不正确，期望3个，实际{len(user_requirements)}个"
        assert len(ai_documents) == 3, "AI文档数量不正确"
        assert len(comparison_results['covered']) == 3, f"已覆盖需求数量不正确，期望3个，实际{len(comparison_results['covered'])}个"
        assert len(comparison_results['missing']) == 0, "未覆盖需求数量不正确"
...
def main():
    """运行所有测试。"""
    print("开始端到端测试 spec-verifier skill...")
    print()
