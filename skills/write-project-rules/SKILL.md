...
基于架构约束，列出"如果 AI 不知道这条规则，会写出什么错误代码"

**方法**：
1. 对于每个架构约束，问："AI 可能会怎么违反这条约束？"
2. 在现有代码中查找反例（验证规则的必要性）

**示例**：

架构约束："features 之间禁止直接依赖"
...
→ 验证：在现有代码中搜索 `import.*from.*@/features/` 模式
- 如果找到反例 → 说明这条规则很重要
- 如果没有反例 → 说明现有代码已经遵守
