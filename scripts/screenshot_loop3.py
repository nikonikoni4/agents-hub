...
1. 整体主界面 - 完整的页面布局
2. 左侧边栏 - 会话列表区域
3. 聊天消息区域 - 消息展示区域（包含新的气泡边框效果）
4. 右侧边栏 - Agent调用和任务面板
5. 聊天输入框 - Loop 1 修改后的最新状态
"""
...
with sync_playwright() as p:
    ...
    # 截图1: 整体主界面
    page.screenshot(path=f"{OUTPUT_DIR}/loop3_01_overview.png", full_page=True)
    print("截图1完成: 整体主界面")

    # 截图2: 左侧边栏
    left_sidebar = page.locator('[class*="leftSidebar"]')
    if left_sidebar.count() > 0:
        left_sidebar.first.screenshot(path=f"{OUTPUT_DIR}/loop3_02_left_sidebar.png")
        print("截图2完成: 左侧边栏")
    else:
        print("未找到左侧边栏")
    ...
    if chat_messages.count() > 0:
        chat_messages.first.screenshot(path=f"{OUTPUT_DIR}/loop3_03_chat_messages.png")
        print("截图3完成: 聊天消息区域")

        # 截图3.1: Agent消息气泡特写（展示边框效果）
        agent_messages = page.locator('[class*="messageAgent"]')
        if agent_messages.count() > 0:
            agent_messages.first.screenshot(path=f"{OUTPUT_DIR}/loop3_03_1_agent_bubble.png")
    ...
    if right_sidebar.count() > 0:
        right_sidebar.first.screenshot(path=f"{OUTPUT_DIR}/loop3_04_right_sidebar.png")
        print("截图4完成: 右侧边栏")
    else:
        print("未找到右侧边栏")

    # 截图5: 聊天输入框（Loop 1 修改后的最新状态）
    chat_input = page.locator('[class*="chatInputContainer"]')
    if chat_input.count() > 0:
        chat_input.first.screenshot(path=f"{OUTPUT_DIR}/loop3_05_chat_input.png")
        print("截图5完成: 聊天输入框")

        # 截图5.1: 输入框按钮特写
        input_wrapper = page.locator('[class*="chatInputWrapper"]')
        if input_wrapper.count() > 0:
            input_wrapper.first.screenshot(path=f"{OUTPUT_DIR}/loop3_05_1_input_buttons.png")
