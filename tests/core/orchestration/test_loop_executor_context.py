...
def test_build_loop_message_uses_loop_message_type_and_metadata():
    ...
    assert message.metadata["loop_context"] == message.content
    assert message.metadata["is_loop_message"] is True
...
async def test_save_loop_result_uses_loop_chat_format():
    ...
    executor = LoopExecutor(loop=_make_loop([node], current_iteration=2), runtime=runtime)
