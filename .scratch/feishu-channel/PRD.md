...
29. 作为飞书用户，我希望通过 `/default <agent_name>` 命令指定群聊中的默认对话 Agent，以便无需每次 @agent 即可发送消息
    - 仅在群聊状态下可用，非群聊状态返回错误提示
    - 默认对话对象状态持久化，重启后保持
    - 消息回复时底部显示当前默认对话对象和群聊成员列表

### 单聊同步（暂不实现）

30. 作为飞书用户，我希望重启后能够增量同步单聊错过的消息，以便了解之前发生的内容
    - **暂不实现原因**：单聊数据不在 agents-hub 中存储，实现复杂度较高
    - **后续**：待单聊数据存储方案确定后再实现

### 媒体处理（可选/分期）

31. 作为飞书用户，我希望 bot 能够接收图片消息，以便处理图片内容
32. 作为飞书用户，我希望 bot 能够接收文件消息，以便处理文件内容
33. 作为飞书用户，我希望 bot 能够上传图片，以便返回图片结果
34. 作为飞书用户，我希望 bot 能够上传文件，以便返回文件结果
