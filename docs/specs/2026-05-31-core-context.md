...
last_updated: AgentSessionInfo 新增 token 字段
...
| ---- | -------- |
| 1.0 | 创建 spec 初稿 |
| 1.1 | AgentSessionInfo 新增 token 字段 |
...
2. **上下文管理**（GroupChatContext）：协调消息管理、session 管理和上下文压缩
3. **持久化**（GroupChatRepository）：文件读写和并发控制
...
**Session 更新规则**：
- 首次出现的 Agent → 创建新的 AgentSessionInfo
...
GroupChatRepository 负责三种文件的读写，使用独立的 asyncio.Lock 保护写操作：

| 文件 | 格式 | 锁 | 说明 |
|------|------|-----|------|
| `<id>.jsonl` | JSONL | _session_lock | 消息历史，首行为 meta_data |
| `agent_session_state.json` | JSON | _agent_state_lock | Agent session 映射（含 token 字段） |
...
- 存储路径：`local_data/teams/<sanitized_project_path>/<group_chat_id>/`
...
- orchestration 层的 GroupChat 创建并持有 GroupChatContext 实例
