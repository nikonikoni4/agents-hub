...
获取历史: GET /api/v1/group-chats/{group_chat_id}/messages?limit=50&offset=0
  → 分页读取消息历史
  → 返回消息列表
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| content | str | 是 | 消息内容（min_length=1，禁止包含 @ 符号） |
| send_to | str | 是 | 目标角色名（如 'manager' 或具体 worker） |
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| limit | int | 50 | 返回消息数量上限（1-200） |
| offset | int | 0 | 跳过前 N 条消息 |
