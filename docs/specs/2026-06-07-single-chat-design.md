...
class SessionFileNotFoundError(Exception):
    """Session 文件不存在"""
    def __init__(self, session_id: str, platform: AgentPlatform):
        self.session_id = session_id
        self.platform = platform
        super().__init__(f"Session file not found: {session_id} ({platform})")
```
...
3. 调用 agent_bridge.execute(fork_from=main_session, ...)
4. CLI 返回新 session_id → 保存到索引
```
...
2. 直接使用 main_session（不 fork）
3. 后续消息：agent_bridge.execute(session_id=main_session, ...)
4. 消息不进入群聊历史（纯私聊）
```
...
```python
def resolve_session_path(session_id: str, platform: AgentPlatform) -> Path:
    """根据 session_id 和平台类型解析 session 文件路径"""
    
    if platform == AgentPlatform.CLAUDE:
        # Claude Code: ~/.claude/sessions/{session_id}.json
        ...
        for f in base_dir.rglob("*.jsonl"):
            # 从文件名提取 UUID（最后一段）
            stem = f.stem  # rollout-2026-06-07T21-40-59-019ea250-b882-74c3-99ce-f7d4e3300e37
            if session_id in stem:
                session_file = f
                break
        if not session_file:
            raise SessionFileNotFoundError(session_id, platform)
