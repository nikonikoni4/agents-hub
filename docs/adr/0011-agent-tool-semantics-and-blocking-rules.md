...
abstract: 收窄 report_progress 为任务汇报工具，所有成果/问题/风险通过 complete_task 汇报；阻塞判定标准为"影响范围是否超出任务边界"
status: decided
---
...
report_progress 的使用边界不清晰，容易被 Agent 滥用（长篇汇报、频繁进度更新），导致群聊消息膨胀。同时缺乏阻塞判定标准，Worker 遇到问题时不知道应该自己处理还是上报。

### 讨论范围

- report_progress 和 complete_task 的语义边界
- Worker 什么情况下算"阻塞"
...
- report_progress 定义为"在群聊中公开发言"，过于宽泛
- Worker 没有明确的阻塞判定标准
...
### 方案 B：收窄 report_progress + 阻塞判定规则

将 report_progress 收窄为"任务汇报"工具，所有成果/问题/风险通过 complete_task 汇报。定义明确的阻塞判定标准。
...
**劣势**
- report_progress 的使用场景变得很窄
- 需要在提示词中详细说明阻塞判定规则

### 方案 C：去掉 report_progress，只保留 complete_task

完全移除 report_progress，所有输出都通过 complete_task。
...
选择 **方案 B：收窄 report_progress + 阻塞判定规则**。

## 决策原因

1. **没有中断机制**：即使 report_progress 通知了 Manager，Manager 也无法实时阻止 Worker（延迟问题）。所以执行过程中的通知对任务控制没有实际作用。
