...
├── teams/                         # 团队管理
├── channels/                      # 外部渠道适配（微信等）
│   └── wechat/                    # 微信渠道（QR 登录、消息轮询）
│
├── tools/                         # 工具目录（硬编码所有可用工具的分组和描述）
│
├── agent_bridge/                  # Agent 平台适配层
│   ├── executors/                 # 各平台执行器（claude、codex、opencode）
│   │   ├── claude.py              # Claude CLI 执行器
│   │   ├── codex.py               # Codex CLI 执行器
│   │   ├── opencode.py            # OpenCode CLI 执行器
...
| utils/ | 核心工具层（markdown_injector、path_utils），**无层级依赖** | - |
