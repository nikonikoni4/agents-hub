...
│   │   ├── task.py                # Task/TaskList 数据模型
│   │   └── task_manager.py        # 任务管理器（CRUD + 持久化）
│   │
│   ├── context/                   # 🟠 上下文层（依赖 foundation）
│   │   ├── group_chat_session.py  # 群聊会话
│   │   ├── group_chat_context.py  # 群聊上下文 Facade
│   │   ├── group_chat_runtime.py  # 群聊运行时（State + Repository）
│   │   ├── group_chat_runtime_state.py # 运行时内存状态
│   │   ├── group_chat_repository.py # 群聊持久化层
│   │   ├── group_metadata.py      # 群聊元数据
...
│
├── teams/                         # 团队管理模块（独立于 core/orchestration）
│   ├── models.py                  # TeamInfo 数据模型
...
│  - MessageRouter: 消息路由      - GroupChatContext: Facade   │
│  - AgentCall: 调用记录          - GroupChatRuntime: 运行时   │
│  - AgentCallManager: 调用管理   - GroupChatRuntimeState: 状态│
│  - Task/TaskList: 任务模型      - GroupChatRepository: 持久化│
│  - TaskManager: 任务管理        - GroupMetadata: 元数据      │
...
#### 2. communication/ - 通信层
**职责**：消息路由、调用管理和任务管理，**只依赖 foundation**
...
- `task.py`: 任务模型
  - `Task`: 单个任务（PENDING → RUNNING → COMPLETED/FAILED）
  - `TaskList`: 任务列表（ACTIVE / ARCHIVED）

- `task_manager.py`: 任务管理器
  - `TaskManager`: Task 的 CRUD 和持久化
...
- `group_chat_context.py`: 群聊上下文 Facade
  - `GroupChatContext`: 对外统一访问接口，底层委托给 GroupChatRuntime

- `group_chat_runtime.py`: 群聊运行时
  - `GroupChatRuntime`: 管理 State（内存状态）和 Repository（持久化层）

- `group_chat_runtime_state.py`: 运行时内存状态
  - `GroupChatRuntimeState`: 持有 session、agent_member_infos、compact_history

- `group_chat_repository.py`: 群聊持久化层
  - `GroupChatRepository`: 文件读写和并发控制（锁保护）

- `group_metadata.py`: 群聊元数据
  - `GroupMetadata`: group_chat_id、project_path、group_type 等

- `agent_context.py`: Agent 上下文
  - `AgentContext`: 为 Agent 提供增量加载的上下文
...
- `group_chat_manager.py`: 群聊管理器
  - `GroupChatManager`: 全局管理所有 GroupChat 实例（单例）
...
- `assign_tasks_to_team`: Manager 向团队成员分配任务
- `archive_task_list`: 归档当前任务列表
- `check_agent_call`: 查询 Agent 调用状态
- `speak_in_group_chat`: Agent 在群聊中发言（支持 @mention）
- `finish_agent_call`: 结束当前 Agent 调用
...
**文件**：
- `app.py`: FastAPI 主应用（异常处理、路由注册、生命周期管理）
- `routes/`: 路由定义（group_chat、roles、skills、teams、config）
- `schemas/`: Pydantic 请求/响应模型
- `services/`: 业务服务层（route → service → manager 分层）
- `websocket/`: WebSocket 端点
...
- `docker/`: Docker 容器支持
  - `container_manager.py`: 容器生命周期管理（创建、启动、停止）
...
│   │   │   ├── hooks/              # useChatMessages 等
│   │   │   ├── store/              # chatStore.ts（Zustand）
│   │   │   └── types.ts
│   │   │
│   │   ├── session/                # 会话管理（群聊列表、创建、切换）
│   │   │   ├── components/         # SessionList、CreateGroupChatDialog 等
│   │   │   ├── hooks/
│   │   │   ├── store/
│   │   │   └── types.ts
│   │   │
...
│   │
│   ├── shared/                     # 共享资源
│   │   ├── components/             # 通用组件（MarkdownRenderer 等）
│   │   ├── adapters/               # 数据适配器
│   │   └── types/                  # 全局类型定义
│   │
│   ├── layouts/                    # 布局组件
