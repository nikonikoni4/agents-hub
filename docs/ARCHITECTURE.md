...
**职责**：提供 MCP Tools，让 Agent 之间可以通过 MCP 协议通信
...
- `server.py`: MCP Server 主入口
  - 使用 FastMCP 框架
  - 注册 MCP Tools（call_agent、list_agents、get_history 等）
  - 处理 Agent 的 tool_use 请求
...
**职责**：适配不同的 Agent 平台（Claude Code、Codex 等）

**文件**：
- `bridge.py`: 统一的 Agent 调用接口
  - `AgentBridge`: 提供统一的 `execute()` 方法

- `executors/`: 各平台的执行器
  - `claude.py`: Claude Code 执行器
  - `codex.py`: Codex 执行器

- `parsers/`: 各平台的响应解析器
  - `claude.py`: Claude Code 响应解析
  - `codex.py`: Codex 响应解析

- `models.py`: 数据模型
  - `AgentResult`: Agent 执行结果
  - `AgentPlatform`: Agent 平台枚举
...
```
User (前端)
  → WebSocket: {type: "message", to: "Leader", content: "..."}
    → API Server (FastAPI)
      → call_agent(group_chat_id, "user", "Leader", content)
        → MessageRouter.send_message()
          → Leader.message_queue
            → Leader.run() 处理消息
              → AgentBridge.execute()
                → Claude Code 执行
                  → 返回 AgentResult
                    → 保存到 GroupChatContext
                      → WebSocket 推送给前端
```
...
```
Agent A (Claude Code)
  → tool_use: call_agent(send_to="B", content="...")
    → MCP Server (STDIO)
      → call_agent()
        → MessageRouter.send_message()
          → Agent B.message_queue
            → Agent B.run() 处理消息
              → AgentBridge.execute()
                → Codex 执行
                  → 返回 AgentResult
                    → 保存到 GroupChatContext
                      → 如果 need_response=True，回复给 Agent A
```
