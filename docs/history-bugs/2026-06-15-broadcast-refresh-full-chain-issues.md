...
| # | 问题 | 置信度 | 类型 | 修复难度 | 状态 |
|---|------|--------|------|----------|------|
| 1 | 双重广播 `_handle_fallback_close()` | 85 | DRY 违反 | 低 | ✅ 已修复 (60734e5) |
| 2 | N+1 广播 `add_members_to_group_chat()` | 85 | DRY 违反 | 低 | 暂不修复 |
| 3 | 请求风暴（10 并发 / 6 冗余） | 78 | 性能 | 中 | ✅ 已修复 (60734e5) |
| 4 | broadcast 并发不安全 | 75 | 并发 bug | 低 | ✅ 已修复 |
| 5 | 无心跳/死连接 | 75 | 健壮性 | 低 | ✅ 已修复 |
| 6 | 重连窗口期 refresh 丢失 | 75 | 可靠性 | 低 | ✅ 已修复（同时解决 #7） |
| 7 | 断连期间 refresh 丢失 | 65 | 可靠性 | 低 | ✅ 被 #6 覆盖 |
