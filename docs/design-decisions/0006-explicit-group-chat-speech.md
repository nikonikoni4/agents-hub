...
abstract: 将群聊发言从 Agent.run() 出口 A/B 的隐式自动写入，改为显式 MCP 工具调用；普通公开发言使用 speak_in_group_chat，需要回复的 AgentCall 使用 complete_task 闭环，Agent 调用方通过完成通知唤醒，user 调用方通过群聊消息和 refresh 感知结果
status: decided
---
...
测试阶段的 MCP 工具集已有 `call_agent` `assign_tasks_to_team` `archive_task_list` `check_agent_call`。新增 `speak_in_group_chat` 和 `complete_task` 后总共 6 个，数量仍可控；更重要的是每个工具职责边界更清晰。
