...
- 内容摘要：群聊发言从隐式自动写入（出口 A/B）改为显式 MCP 工具调用。普通公开发言使用 speak_in_group_chat；需要回复的 AgentCall 使用 complete_task 闭环。目的是分离 LLM 私下执行文本、公开群聊发言和调用状态闭环
...
- 状态：decided
- 触发规则：当设计或修改 speak_in_group_chat/complete_task 的使用边界、Worker 阻塞判定规则、Manager 阻塞处理流程时阅读
- 内容摘要：收窄 speak_in_group_chat 为任务汇报工具，所有成果/问题/风险通过 complete_task 汇报；阻塞判定标准为"影响范围是否超出任务边界"
