...
abstract: 收窄 speak_in_group_chat 为任务汇报工具，所有成果/问题/风险通过 finish_agent_call 汇报；阻塞判定标准为"影响范围是否超出任务边界"
status: decided
---
...
- speak_in_group_chat 和 finish_agent_call 的语义边界
- Worker 什么情况下算"阻塞"
...
将 speak_in_group_chat 收窄为"任务汇报"工具，所有成果/问题/风险通过 finish_agent_call 汇报。定义明确的阻塞判定标准。

**优势**
- 语义清晰，Agent 不需要在两个工具之间纠结
- finish_agent_call 成为唯一的信息汇总出口，Manager 不需要同时关注两个渠道
...
### 方案 C：去掉 speak_in_group_chat，只保留 finish_agent_call

完全移除 speak_in_group_chat，所有输出都通过 finish_agent_call。
...
- finish_agent_call 需要 call_id，不是所有场景都有
...
2. **finish_agent_call 是唯一可靠的信息汇总出口**：它携带 call_id，能激活原调用方，有明确的状态标记（success/failed）。所有成果、问题、风险都应通过这个渠道汇报。
...
Worker 遇到以下情况，用 finish_agent_call 标记失败（success=false）：
