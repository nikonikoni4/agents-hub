...
**日期**：2026-06-02  
**任务背景**：为 AgentMemberInfo 添加 cwd 参数，并设计群聊元数据持久化方案
...
用户想要：
1. 在 `AgentMemberInfo` 上添加 `cwd` 参数，表示每个 role 的 CLI 命令启动位置
...
✅ `AgentMemberInfo` 已有 `cwd` 字段（由 `agents_bridge` 已支持）
...
✅ `GroupChat._generate_and_register_tokens()`:
   - 创建新 `AgentMemberInfo` 时，使用 `group_chat_session.group_cwd` 作为默认 `cwd`
...
# 创建 AgentMemberInfo 时使用
AgentMemberInfo(token=token, cwd=default_cwd)
```
...
- `agents_hub/core/context/group_chat_session.py` - AgentMemberInfo, GroupChatSession
