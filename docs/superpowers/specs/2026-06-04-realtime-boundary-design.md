...
当前 WebSocket 连接管理位于 `agents_hub/api/websocket/`。这在只由 API 路由调用时可以工作，但 MCP Server 的 `report_progress` 在写入群聊消息后、`complete_task` 在 AgentCall 闭环后也需要通知前端刷新。如果 MCP 直接依赖 API WebSocket 模块，会形成 `mcp -> api` 的依赖方向，使 API、MCP、WebSocket 三者的职责边界混在一起，并增加循环依赖风险。
...
- MCP 工具在群聊消息或 AgentCall 完成通知改变协作状态后可以触发前端刷新，但不依赖 API 模块。
...
1. MCP tool 完成业务写入：
   - `report_progress` 写入公开群聊消息。
   - `complete_task` 更新 AgentCall 并写入最终群聊回复。
...
- MCP 的 `report_progress` 写入消息后触发 refresh。
- MCP 的 `complete_task` 写入最终回复后触发 refresh。
