...
后端已完成：`complete_task` MCP tool 接收 `web_preview_url` + `web_preview_title`，经 `AgentResult` → `GroupChatSession.add_message()` → 消息 dict → `MessageInfo` Pydantic schema 传递到前端 API。
