...
- **持久化**：`agent_session_state.json` 新增 `agent_token` 字段
...
4. **context 层**：
   - `agent_session_state.json` 新增 `agent_token` 字段
...
**持久化格式（agent_session_state.json）：**
```json
{
  "Manager": {
...
- **spec-core-context** — 持久化机制、agent_session_state.json
