...
- `agents_hub/core/context/group_chat_session.py` - AgentSessionInfo 新增 use_docker 字段
...
## Task 3: 扩展 AgentSessionInfo 数据模型
...
def test_agent_session_info_with_use_docker():
    """测试设置 use_docker"""
    info = AgentSessionInfo(use_docker=True)
...
Expected: FAIL - AttributeError: 'AgentSessionInfo' object has no attribute 'use_docker'
...
git commit -m "feat(core): AgentSessionInfo 新增 use_docker 字段"
```
...
def create_mock_agent(use_docker=False, agent_cwd="local_data", project_path="local_data"):
    ...
    session_info = AgentSessionInfo(cwd=agent_cwd, use_docker=use_docker)
