...
class GroupChat:
    ...
    def __init__(
        self, team: Team, group_type: str, project_path: str, group_chat_id: str = str(uuid4())
    ):
...
class GroupChatContext:
    ...
    def update_agent_member_info(self, agent_result: AgentResult):
        ...
        if agent_name not in self.agent_member_info:
            ...
            # 如果 session_id 不同于 main_session，且不在 btw_session 中
                ...
                session_id != agent_member_info.main_session
                and session_id not in agent_member_info.btw_session
            ):
    ...
    async def compact_messages(self, agent_info: dict[str, str]):
        ...
        compact_record = {
            ...
            "content": {"summary": summary, **agent_specific},
        }
...
def create_defaute_role():
    ...
    role_manager.create_role("llm_call", AgentPlatform.CLAUDE, type=RoleType.LEADER)
    role_manager.create_role("llm_call_codex", AgentPlatform.CODEX, type=RoleType.LEADER)
...
class Agent:
    def __init__(self, role: Role, group_chat_context: GroupChatContext | None = None):
        ...
        )  # 这里很奇怪，这些信息应该都直接作为类成员比较好啊，先不优化，role和agentbriged之后在优化
    ...
    def main_session_id(self):
        if self.group_chat_context.agent_member_info.get(self.name):
            if self.group_chat_context.agent_member_info[self.name].main_session:
                ...
                print(
                    f"warning : {self.name}在当前群聊中无历史记录"
                )  # 这里的print 需要替换为具体的logger
        else:
            print(
                f"warning : 当前群聊无{self.name}的main session记录 [ 如果是初始化会话 忽略改警告]"
            )
        return None
...
class GroupChatManager:
    ...
    def register(self, group_chat_id: str, group_chat: GroupChat):
        if (
            group_chat_id
            and isinstance(group_chat_id, str)
            and group_chat
            and isinstance(group_chat, GroupChat)
        ):
